        ...
        <div className="md:flex">
          ...
          <div className="md:w-1/2 lg:flex lg:flex-col lg:justify-center lg:items-center lg:pl-5">
            <div className="p-5 bg-white text-center my-5 hidden lg:block w-full">
              <h2 className="text-xl">Real Estates & Homes for Sale</h2>
            </div>
            ...
            productList.map((item) => (
              <Link key={item[0]} to={`/listing/${item[0]}`}>
              <div key={item[0]} className="border p-4 mb-4">
                <p>Lsid: {item[0]}</p>
                <p>Lsdate: {item[1]}</p>
                <p>Lsdays_since_published: {item[2]}</p>
                <p>Lsprice: {item[3]}</p>
                <p>Lsprice_per_area: {item[4]}</p>
                <p>Lsfrom_who: {item[5]}</p>
                <p>Lsland_title_status: {item[6]}</p>
                <p>Lsdescription: {item[7]}</p>
                <p>Admin Id: {item[8]}</p>
              </div>
              </Link>
  ))
